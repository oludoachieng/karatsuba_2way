...
// open a file in write mode.
   ofstream outfile;
   outfile.open("output_naiv.txt");

   // write inputted data into the file.
   outfile <<multiply(str1, str2)<<endl;
...
string sumA=findSum(msb1_str, lsb1_str);
string sumB=findSum(msb2_str, lsb2_str);
string V_AB=multiply(sumA,sumB);
// open a file in write mode.
   outfile.open("truncated.txt");

   // write inputted data into the file.
   outfile <<multiply(msb1_str, msb2_str)<<endl ;
   outfile <<multiply(lsb1_str, lsb2_str)<<endl ;
   outfile <<V_AB<<endl ;
   // close the opened file.
   outfile.close();
...
//Open and read input file
readvar.open("truncated.txt");
if(readvar.is_open()){
    if(readvar.good()){
        getline(readvar,inp1);
        getline(readvar,inp2);
        getline(readvar,inp3);
    }
}
infile.close();

string diffAB=findDiff(inp3,inp1);
diffAB=findDiff(diffAB,inp2);
...
string soln=findSum(inp1,diffAB);
       soln=findSum(soln,inp2);

       // open a file in write mode.
   ofstream outfile2;
   outfile2.open("output_kara.txt");
